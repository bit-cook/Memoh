...
- The canonical workspace image is built from `docker/Dockerfile.workspace`; per-bot managed dependencies keep metadata under `/data/.memoh/deps` via `internal/workspacedeps/`; `container.dependency_store_root` can place payloads and caches on local storage.
