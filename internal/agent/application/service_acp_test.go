...
func TestStreamChatWSRoutesACPRuntimeSessionToACPPool(t *testing.T) {
	...
	if err := resolver.StreamChatWS(
		...
		ChatRequest{
			...
			Attachments: []ChatAttachment{{
				Type:   "image",
				Base64: "data:image/png;base64," + base64.StdEncoding.EncodeToString(validVisionPNG(t)),
				Mime:   "image/png",
				Name:   "screenshot.png",
			}},
	...
	if len(pool.input.Images) != 1 || pool.input.Images[0].Data != base64.StdEncoding.EncodeToString(validVisionPNG(t)) || pool.input.Images[0].MimeType != "image/png" {
